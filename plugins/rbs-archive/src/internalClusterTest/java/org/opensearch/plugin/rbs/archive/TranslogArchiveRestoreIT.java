...
/**
 * Integration tests for translog archive restore: index with archive enabled,
 * then restore from remote and verify doc count and content.
 *
 * <p>Uses the "tar" translog strategy (via INDEX_REMOTE_STORE_TRANSLOG_STRATEGY_SETTING)
 * to enable TAR-based batch uploads. Tests verify that archives are uploaded correctly
 * and can be reliably restored to the same or different nodes.
 */
@ThreadLeakFilters(filters = TranslogArchiveTimerThreadLeakFilter.class)
@OpenSearchIntegTestCase.ClusterScope(scope = OpenSearchIntegTestCase.Scope.TEST, numDataNodes = 0)
public class TranslogArchiveRestoreIT extends BaseRemoteStoreRestoreIT {

    private static final String INDEX_NAME = "test-restore";
    ...
    /**
     * Index with archive enabled, stop primary, restore from remote, verify hit count.
     */
    public void testRestoreFromRemoteWithArchiveUploadEnabled() throws Exception {
        internalCluster().startClusterManagerOnlyNode();
        // Set buffer interval before data nodes start so batch coordinator picks 50ms at doStart()
        client().admin()
            .cluster()
        ...
        // Use emptyTranslog=false so last iteration leaves uncommitted ops for archive to upload
     ...
     * verify all batches' data recovered from multiple archive TARs.
     */
    public void testRestoreFromMultipleTranslogArchivesOnDifferentNode() throws Exception {
        ...
        // Multiple indexing iterations to create multiple archive TARs
    ...
    public void testRestoreVerifiesDocumentContentNotJustCount() throws Exception {
        ...
        // Batch 2: NOT flushed → stays in translog → must be recovered from archive TAR
        Map<String, String> batch2 = indexKnownDocs(INDEX_NAME, 20);

        waitForTranslogArchiveUpload();

        // Kill primary — batch2 only exists in the archive TAR
        internalCluster().stopRandomNode(InternalTestCluster.nameFilter(primaryNodeName(INDEX_NAME)));
        ensureRed(INDEX_NAME);
    ...
    /**
     * Same-node restart: primary restarts and must recover its own archive TARs from remote.
     * Verifies the node-specific path structure is correct (same node uploads and downloads its own TARs).
     */
    public void testSameNodeRestartRecoversFromArchive() throws Exception {
        ...
        // Restart the SAME node (not kill) — node comes back with same nodeId → reads its own archive dir
     ...
     *
     * Exercises scenario where batch1 was uploaded as per-shard translog files (archive OFF),
     * and batch2 was uploaded as a TAR archive (archive ON). Restore must recover all.
     */
    public void testRestoreAfterArchiveToggleOffToOn() throws Exception {
        ...
        Settings indexSettings = Settings.builder()
            .put(remoteStoreIndexSettings(0, 1))
            .put(IndexSettings.INDEX_REMOTE_STORE_TRANSLOG_STRATEGY_SETTING.getKey(), "")
            .build();
        createIndex(INDEX_NAME, indexSettings);
        ensureGreen(INDEX_NAME);

        // Batch 1 with archive OFF — uploaded as per-shard translog files
        Map<String, String> batch1 = indexKnownDocs(INDEX_NAME, 20);
        flushAndRefresh(INDEX_NAME);
        ...
        client().admin()
            ...
            .setSettings(Settings.builder().put(IndexSettings.INDEX_REMOTE_STORE_TRANSLOG_STRATEGY_SETTING.getKey(), "tar"))
            .get();
        ...
        // Kill primary — batch2 in TAR, batch1 in per-shard translog (both flushed to segments)
        internalCluster().stopRandomNode(InternalTestCluster.nameFilter(primaryNodeName(INDEX_NAME)));
        ensureRed(INDEX_NAME);
     ...
     * Path layout: txlog/{yyyyMMdd}/{HHmm}/*.tar
     */
    ...
    /**
     * Searches the repo root for any archive file (.tar) under the txlog/ directory.
     * Path: txlog/{day}/{minute}/*.tar
     */
        ...
        try {
            Files.walkFileTree(repoRoot, Set.of(FileVisitOption.FOLLOW_LINKS), Integer.MAX_VALUE, new SimpleFileVisitor<>() {
                ...
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
                    ...
                    // Path: txlog/{day}/{minute}/*.tar (2 levels deep from txlog)
    ...
    /**
     * Check for any *.tar archive under dir, recursing {@code levelsDeep} levels.
     *
                    ...
                    for (Path minuteDir : minuteDirs) {
                        ...
                        try (DirectoryStream<Path> blobs = Files.newDirectoryStream(minuteDir, "*.tar")) {
                            for (Path blob : blobs) {
                                ...
                                result.add("txlog/" + dayDir.getFileName() + "/" + minuteDir.getFileName() + "/" + blob.getFileName());
                            }
                        }
                    }
                }
    ...
    private static void assertTarPathStructure(String relPath) {
        ...
        Pattern pattern = Pattern.compile("txlog/\\d{8}/\\d{4}/\\d{2}\\.\\d{3}\\.[a-zA-Z0-9]{8}\\.tar");
        assertTrue(
