 ...
 *
 * <p>During recovery, multiple shards from the same node may need to read index entries from the
 ...
 * @opensearch.internal
 */
final class TranslogArchiveIndexCache {
     ...
     */
    TranslogArchiveIndexCache(int maxEntries) {
        if (maxEntries <= 0) {
            throw new IllegalArgumentException("maxEntries must be > 0, got: " + maxEntries);
        }
        this.cache = Collections.synchronizedMap(
            new LinkedHashMap<String, List<TarArchiveBuilder.EntryLocation>>(maxEntries, 0.75f, true /* accessOrder = LRU */) {
                @Override
